...
if ~meta.sorted
    ...
    if meta.copyfiles
        for fileIdx = 1:length(meta.taskAlias)
            % copy neural data
            copyfile(fullfile(meta.sortedfolder,sprintf('%s_%s.mat',meta.neuralPrefix,meta.taskAlias{fileIdx})),...
                fullfile(meta.workingfolder,'preCDS','Final'))
            if exist('altMeta','var') && ~isempty(altMeta.array)
                copyfile(fullfile(meta.sortedfolder,sprintf('%s_%s.mat',altMeta.neuralPrefix,meta.taskAlias{fileIdx})),...
                    fullfile(meta.workingfolder,'preCDS','Final'))
            end
        end
    else
